---
title: Aleks Trpkovski — Shoreline - F96 Coral Sea Wireless Mechanical Keyboard
description: Discover Shoreline, a bold 96% custom mechanical keyboard built on the F96 Coral Sea. Featuring smooth Cherry MX Red switches, navy blue, white, and orange Cherry profile PBT keycaps, RGB lighting, and wired + wireless connectivity for a coastal-inspired typing experience.
image: https://res.cloudinary.com/suv4o/image/upload/q_auto,f_auto,w_1200,e_sharpen:100/v1777637182/blog/the-keyboard-lab/_DSC5608_evraeg.jpg
keywords:
    - Mechanical Keyboards
    - Custom Keyboards
    - Keyboard Builds
    - DIY Keyboards
    - Mechanical Switches
    - Keycaps
    - Coiled Cables
    - Keyboard Lab
    - Keebs
    - Desk Setup
    - Frontend
    - Developer Gear
    - shoreline keyboard
    - F96 Coral Sea
    - Akko F96
    - 96% keyboard
    - Cherry MX Red switches
    - Cherry profile keycaps
    - PBT keycaps
    - wireless mechanical keyboard
    - RGB backlit keyboard
    - navy blue keyboard
    - coastal keyboard
    - nautical keyboard
    - hot-swappable keyboard
type: page
shorelineSpecs:
    - id: 1
      title: "Type of Keyboard"
      description: |
          <ul class="list-disc pl-5">
              <li>F96 Coral Sea (96% / 1800 layout)</li>
              <li>Hot-swappable PCB</li>
              <li>Wired + Wireless connectivity</li>
              <li>Plastic case in navy blue</li>
              <li>RGB LED backlighting</li>
          </ul>
    - id: 2
      title: "Type of Switches"
      description: |
          <ul class="list-disc pl-5">
              <li>Cherry MX Red (Linear, 45gf actuation force)</li>
              <li>Smooth, light typing feel for fast, fatigue-free use</li>
          </ul>
    - id: 3
      title: "Type of Keycaps"
      description: |
          <ul class="list-disc pl-5">
              <li>Cherry Profile</li>
              <li>PBT plastic</li>
              <li>Navy blue, white, and orange colourway</li>
              <li>Coastal-inspired nautical aesthetic</li>
          </ul>
    - id: 4
      title: "Type of Cable"
      description: |
          <ul class="list-disc pl-5">
              <li>Detachable USB-C cable for wired use</li>
              <li>Wireless mode available for cable-free setups</li>
          </ul>
    - id: 5
      title: "Mods"
      description: |
          <ul class="list-disc pl-5">
              <li>Stabilisers removed and lubed with dielectric grease</li>
              <li>Cherry MX Reds left stock for an authentic, factory-smooth feel</li>
              <li>Keycap and switch combo tuned for a crisp, classic Cherry sound profile</li>
          </ul>
shorelineTitle: "🌊 Shoreline"
shorelineDescription: "Bold, classic, and unmistakably coastal — <strong>Shoreline</strong> is a 96% custom mechanical keyboard built on the F96 Coral Sea. Its navy blue case pairs beautifully with crisp white and vibrant orange Cherry profile PBT keycaps, evoking the colours of a sun-soaked harbour. Powered by smooth Cherry MX Red switches, every keystroke is light, fast, and effortless — perfect for marathon typing and gaming sessions alike. With wired and wireless modes plus RGB lighting, <strong>Shoreline</strong> blends timeless aesthetics with modern flexibility, making it a confident daily driver for anyone who loves a nautical edge."
shorelineTags: "nautical, bold, coastal, classic"
shorelineImage: "https://res.cloudinary.com/suv4o/image/upload/q_auto,f_auto,w_900,e_sharpen:100/v1777637182/blog/the-keyboard-lab/_DSC5608_evraeg.jpg"
---

::TheKeyboardLabPage
::
